...
function ChatRoomInput({ setChatMessages }) {
  ...
  const { targetLanguage} = useChatContext();
  ...
  const handleInputSubmit = (e) => {
    ...
    postUserMessage(
      ...
      (data) => {
        // update own room message
        setChatMessages((prev) => [...prev, { ...data.data, avatarImage: user.avatarImage }]);

        // Use socket to notify the other party immediately
  ...
  const handleKeyUp = () => {
    //Only emit if the typing is different
  ...
  useEffect(() => {
    if (typingNotify) {
      ...
      setShowNotify(typing && isChatting); // Only typing is displayed in chat
    } else {
      setShowNotify(false);
    }
  }, [typingNotify, chatId]);
