...
function ChatContextProvider({ children }) {
  ...
  // When there is new information, update contact with the latest information
  useEffect(() => {
    if (messageData) {
  ...
  // Notify the other party that you have read it
  const updateMessageStatusToRead = (chatId, type) => {
    // API Update the message sent by the other party as read
    updateReadStatus({
      method: 'PUT',
    ...
    // socket - Notify the other party that you have read it
  ...
  const handleChatSelect = async (selected) => {
    if (selected._id !== chatId) {
      if (selected.chatType === 'room') {
        socketEmitEvent(socket).enterChatRoom({ roomId: selected._id, message: `${user.name} joined the chat` });
      }
      if (chatInfo?.chatType === 'room') {
        socketEmitEvent(socket).leaveChatRoom({ roomId: chatId, message: `${user.name} left chat` });
      }
